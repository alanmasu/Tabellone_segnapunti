/*
    ...
    Note
     ...
     - Sleep modes
     - Avanzamento veloce
     - Modalità automatica orologio dopo 30s dalla disconessione
     - Modalità Orologio con RTC DS3231 in I2C

    PRIMA PROVA ESP-NOW
    TO DO LIST:
     - Cambio di protocollo           [WORKING IN PROGGRES] [WORKING] [TO TRY]
     - OTA                            [TO DO]
     - Valutare il PowerFail detector [TO DO]
...
#define ESP_NOW_MAX_TIMEOUT 10 * 1000UL //10 secondi
...
bool modeImpostata = false;   //Per ripristino automantico tabellone alla riconnessione
...
bool initESP_NOW() {
  ...
  // Settaggio callback in ricezione
...
bool checkNOWConnection() {
  return millis() - lastMessageFromNOW > ESP_NOW_MAX_TIMEOUT ? false : true;
}
...
void loop() {
  if (!powerFail_event) { //Se non è in corso un powerfail
    while (Serial.available() > 0) {
      readSerial();
    }
    if (checkNOWConnection()) { //Controlli la connessione
      if (modeImpostata == false && mode == 1) { //Se ti trovi in mod. Orologio allora ti re-imposti a mod. Tab
        mode = 0;
        stateP = HIGH;
        timer2p.detach();
        displayWrite();
      ...
      time_c = millis();
      //dataFromClient = readClient(); DEPRECATED
